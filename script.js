// Firebase Imports (MUST be at the top of a module script for index.html)
// Note: This needs to be available in both index.html and login.html.
// For index.html, the Firebase SDKs are initialized in a <script type="module"> in the head,
  ...
  function initFeatureAccess() {
    ...
    if (btnAddImage && imgPicker) {
      if (!earlyAccess) {
        ...
        if (!document.getElementById("imgPicker")) { // Check if picker exists before recreating
          const existingStickerBar = document.getElementById("stickerBar");
          if (existingStickerBar) {
            const newPicker = document.createElement("input");
            newPicker.id = "imgPicker";
            newPicker.type = "file";
            newPicker.accept = "image/*";
            newPicker.hidden = true;
            existingStickerBar.appendChild(newPicker); // Append to stickerBar, not btnAddImage.parentNode
          }
        }
      }
    }
